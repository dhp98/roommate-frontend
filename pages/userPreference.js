export default function PageWithJSbasedForm() {
    ...
    const handleSubmit = async (event) => {
      ...
      const data = {
        userID : "6382b6a5937aeccafe208b74"
      }

      if (event.target.DOB.value.length) {
        data['DOB'] = event.target.DOB.value
      }
      if (event.target.foodPreference.value.length) {
        data['foodPreference'] = event.target.foodPreference.value
      }
      if (event.target.drinking.value.length) {
        data['drinking'] = event.target.drinking.value
      }
      if (event.target.smoking.value.length) {
        data['smoking'] = event.target.smoking.value
      }
      if (languagesSelected.length) {
        data['languages'] = languagesSelected
      }
      if (event.target.cookingSkills.value.length) {
        data['cookingSkills'] = event.target.cookingSkills.value
      }
      if (event.target.nationality.value.length) {
        data['nationality'] = event.target.nationality.value
      }
    ...
    return (
      ...
      <form onSubmit={handleSubmit}>
        ...
        <input type="date" id="DOB" name="DOB" style={inputStyle}/>
        <br/>
        <label for='studyProgram'><span style={spanStyle}>Study Program: </span></label>
        <select id="studyProgram" name="studyProgram" style={inputStyle}>
        ...
        <label for="foodPreference"><span style={spanStyle}>Food Preference: </span></label>
          <select id="foodPreference" name="foodPreference" style={inputStyle}>
            <option value="" disabled selected>Prefer not to say</option>
          ...
          <select id="drinking" name="drinking" placeholder="drinking" style={inputStyle}>
          <option value="" disabled selected>Prefer not to say</option>
          ...
          <select id="smoking" name="smoking" style={inputStyle}>
           <option value="" disabled selected>Prefer not to say</option>
            <option value="true">Yes</option>
            <option value="false">No</option>
          </select>
          ...
          <select id="languages" name="languages" multiple size='2' style={inputStyle}>
          <option value="" disabled selected>Prefer not to say</option>
          ...
          <select id="cookingSkills" name="cookingSkills" style={inputStyle}>
           <option value="" disabled selected>Prefer not to say</option>
          ...
          <select id="nationality" name="nationality" style={inputStyle}>
           <option value="" disabled selected>Prefer not to say</option>
